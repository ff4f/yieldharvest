...
const Select = React.forwardRef<HTMLButtonElement, SelectProps>(({
    ...
    const getSelectedDisplay = () => {
        ...
        if (multiple) {
            const selectedOptions = options?.filter((opt: SelectOption) => value?.includes(opt?.value));
    ...
    const handleOptionSelect = (option: SelectOption) => {
        if (multiple) {
            ...
            const updatedValue = (newValue as any[])?.includes(option?.value)
                ? (newValue as any[])?.filter((v: any) => v !== option?.value)
                : [...(newValue as any[]), option?.value];
    ...
    const isSelected = (optionValue: string | number) => {
        if (multiple) {
            return (value as any[])?.includes(optionValue) || false;
        }
