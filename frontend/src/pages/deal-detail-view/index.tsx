...
const DealDetailView = () => {
  ...
  useEffect(() => {
    ...
    const fetchDeal = async () => {
      setIsLoading(true);
      try {
        // Fetch invoice data (deal is based on invoice)
        const invoiceResponse = await api.invoice.getById(dealId || '');
        
        // Transform invoice data to deal format
        const transformedDeal = {
          id: invoiceResponse.id,
          title: `${invoiceResponse.description || 'Invoice'} - ${invoiceResponse.supplier?.name || 'Unknown Buyer'}`,
          faceValue: parseFloat(invoiceResponse.amount),
          apr: 12.5, // Default APR - could be calculated based on funding terms
          tenor: Math.ceil((new Date(invoiceResponse.dueDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)),
          riskScore: 8.2, // Default risk score - could be calculated
          expectedYield: parseFloat(invoiceResponse.amount) * 0.125 * (90/365), // Simple yield calculation
          raisedAmount: invoiceResponse.fundings?.reduce((sum: number, f: any) => sum + parseFloat(f.amount), 0) || 0,
          targetAmount: parseFloat(invoiceResponse.amount),
          status: invoiceResponse.status === 'ISSUED' ? 'FUNDING_OPEN' : 
                  invoiceResponse.status === 'FUNDED' ? 'FUNDING_COMPLETE' : 
                  invoiceResponse.status === 'PAID' ? 'COMPLETED' : 'FUNDING_OPEN',
          minInvestment: 5000,
          maxInvestment: 50000,
          invoiceNumber: invoiceResponse.invoiceNumber,
          issueDate: invoiceResponse.createdAt,
          dueDate: invoiceResponse.dueDate,
          settlementDate: invoiceResponse.dueDate, // Assuming settlement on due date
          buyer: invoiceResponse.supplier?.name || 'Unknown Buyer',
          collateral: 'Invoice + Supporting Documents',
          supplier: {
            name: invoiceResponse.supplier?.name || 'Unknown Supplier',
            location: 'Not specified',
            industry: 'Various',
            creditRating: 'A-'
          },
          nft: invoiceResponse.onChainData?.nftInfo ? {
            tokenId: invoiceResponse.onChainData.nftInfo.tokenId,
            serialNumber: invoiceResponse.onChainData.nftInfo.serialNumber.toString(),
            mintTxId: invoiceResponse.mintTransactionId || '',
            metadataUri: invoiceResponse.onChainData.nftInfo.metadata || '',
            metadata: {
              name: `Invoice NFT - ${invoiceResponse.invoiceNumber}`,
              description: `Tokenized invoice: ${invoiceResponse.description}`,
              image: '',
              attributes: [
                { trait_type: 'Face Value', value: `$${invoiceResponse.amount}` },
                { trait_type: 'Invoice Number', value: invoiceResponse.invoiceNumber },
                { trait_type: 'Supplier', value: invoiceResponse.supplier?.name || 'Unknown' },
                { trait_type: 'Status', value: invoiceResponse.status }
              ]
            }
          } : null
        };
  ...
  const handleFunding = async (amount: string | number) => {
    setFundingAmount(amount?.toString());
    setIsFundingModalOpen(true);
  };

  const handleFundingConfirm = (txHash: string) => {
