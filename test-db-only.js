...
async function testDbOnlyInvoice() {
  console.log('🧪 Testing database-only invoice creation...');
  ...
  try {
    ...
    form.append('invoiceNumber', 'DB-TEST-' + Date.now());
    ...
    form.append('description', 'Database-only test invoice');
    form.append('pdfFile', pdfBuffer, {
      filename: 'test-db-invoice.pdf',
      contentType: 'application/pdf'
    });
    
    console.log('📤 Sending database-only request...');
    
    const response = await fetch('http://localhost:3001/api/invoices/test-db-only', {
      method: 'POST',
      body: form,
    ...
    try {
      ...
      if (responseData.data && responseData.data.invoice) {
        ...
        console.log('📋 Amount:', responseData.data.invoice.amount);
      } else {
